...
import ml2json
from mlchemad import IsolationForestApplicabilityDomain
...
def model_applicability_domains(njobs: int = -1):
    """Determine the applicability domains of bioactivity data models of the entire set of bioactivities and of mutants only.
    Output results are stored under DATADIR/modelling
    ...
    for accession in pbar:
        # Determine names of all output files
        out_file_pcm_random_tsv = os.path.join(path, f'pcm_randomsplit_{accession}_applicability_domains.tsv')
        out_file_pcm_mutant_tsv = os.path.join(path, f'pcm_mutantsplit_{accession}_applicability_domains.tsv')
        out_file_pcm_nomutant_random_tsv = os.path.join(path, f'pcm_nomutant_randomsplit_{accession}_applicability_domains.tsv')
        out_file_qsar_random_tsv = os.path.join(path, f'qsar_randomsplit_{accession}_applicability_domains.tsv')
        out_file_qsar_mutant_tsv = os.path.join(path, f'qsar_mutantsplit_{accession}_applicability_domains.tsv')
        out_file_qsar_nomutant_random_tsv = os.path.join(path, f'qsar_nomutant_randomsplit_{accession}_applicability_domains.tsv')
        out_pcm_mutants = [out_file_pcm_random_tsv, out_file_pcm_mutant_tsv]
        out_pcm_no_mutants = [out_file_pcm_nomutant_random_tsv]
        out_qsar_mutants = [out_file_qsar_random_tsv, out_file_qsar_mutant_tsv]
        out_qsar_no_mutants = [out_file_qsar_nomutant_random_tsv]
        all_files = list(chain(out_pcm_mutants, out_pcm_no_mutants, out_qsar_mutants, out_qsar_no_mutants))
        # Skip if all output files already exists
        if all(os.path.exists(f) for f in all_files):
            pbar.update()
        ...
        if not all(os.path.exists(f) for f in out_pcm_mutants):
            ...
            if len(prots) > 0:
                ...
                if pcm_data.shape[0] >= 5:  # Number of folds
                    # Random cross-validation
                    kfold = KFold(n_splits=5, shuffle=True, random_state=1234)
                    ad = IsolationForestApplicabilityDomain(random_state=1234)
                    cv_ad = get_applicability_domain(ad, pcm_data, folds=kfold)
                    # Leave-one-out mutant cross-validation
                    if len(prots.target_id) > 1:
                        gkfolds = GroupKFold(n_splits=len(pcm_target_ids.unique()))
                        loo_ad = get_applicability_domain(ad, pcm_data, folds=gkfolds, groups=pcm_target_ids)
                    else:
                        loo_ad = pd.Series()
                    # Save performances
                    cv_ad.to_csv(out_file_pcm_random_tsv, sep='\t')
                    loo_ad.to_csv(out_file_pcm_mutant_tsv, sep='\t')
        # Run QSAR if not missing any output file
        if not all(os.path.exists(f) for f in out_qsar_mutants):
            if mol_descs is None:  # PCM part was skipped
                # 1) Isolate IDs and dependent variable
            ...
            if qsar_data.shape[0] >= 5:  # Number of folds
                ...
                kfold = KFold(n_splits=5, shuffle=True, random_state=1234)
                ad = IsolationForestApplicabilityDomain(random_state=1234)
                cv_ad = get_applicability_domain(ad, qsar_data, folds=kfold)
                # LOO motant-cross-validation
                if len(prots.target_id) > 1:
                    gkfolds = GroupKFold(n_splits=len(qsar_target_ids.unique()))
                    loo_ad = get_applicability_domain(ad, qsar_data, folds=gkfolds, groups=qsar_target_ids)
                else:
                    loo_ad = pd.Series()
                # Save performances
                cv_ad.to_csv(out_file_qsar_random_tsv, sep='\t')
                loo_ad.to_csv(out_file_qsar_mutant_tsv, sep='\t')
        # Run PCM with mutants removed
        if not all(os.path.exists(f) for f in out_pcm_no_mutants):
            if prot_descs is None:
                # 1) Isolate IDs and dependent variable
            ...
            if prot_descs is not None:
                ...
                if pcm_data.shape[0] >= 5:  # Number of folds
                    ...
                    cv_ad = get_applicability_domain(ad, pcm_data, folds=kfold)
                    # Save performances
                    cv_ad.to_csv(out_file_pcm_nomutant_random_tsv, sep='\t')
        # Run QSAR with mutants removed
        if not all(os.path.exists(f) for f in out_qsar_no_mutants):
            if mol_descs is None:  # PCM part was skipped
                # 1) Isolate IDs and dependent variable
            ...
            if qsar_data.shape[0] >= 5:  # Number of folds
                ...
                cv_ad = get_applicability_domain(ad, qsar_data, folds=kfold)
                # Save performances
                cv_ad.to_csv(out_file_qsar_nomutant_random_tsv, sep='\t')


def model_common_subsets_applicability_domains(njobs: int = -1):
    """Model bioactivities of the similarity-based common subsets only.
    Output results are stored under DATADIR/modelling.
    """
    # Set number of cores
    ...
    for common_set, accession in pbar:
        ...
        if accession_data.empty:
            ...
            continue
        # Determine names of all output files
        out_file_pcm_random_tsv = os.path.join(path, f'pcm_common_subset_randomsplit_{accession}_applicability_domains.tsv')
        out_file_pcm_mutant_tsv = os.path.join(path, f'pcm_common_subset_mutantsplit_{accession}_applicability_domains.tsv')
        out_file_pcm_nomutant_random_tsv = os.path.join(path, f'pcm_common_subset_nomutant_randomsplit_{accession}_applicability_domains.tsv')
        out_file_qsar_random_tsv = os.path.join(path, f'qsar_common_subset_randomsplit_{accession}_applicability_domains.tsv')
        out_file_qsar_mutant_tsv = os.path.join(path, f'qsar_common_subset_mutantsplit_{accession}_applicability_domains.tsv')
        out_file_qsar_nomutant_random_tsv = os.path.join(path, f'qsar_common_subset_nomutant_randomsplit_{accession}_applicability_domains.tsv')
        out_pcm_mutants = [out_file_pcm_random_tsv, out_file_pcm_mutant_tsv]
        out_pcm_no_mutants = [out_file_pcm_nomutant_random_tsv]
        out_qsar_mutants = [out_file_qsar_random_tsv, out_file_qsar_mutant_tsv]
        out_qsar_no_mutants = [out_file_qsar_nomutant_random_tsv, ]
...
def get_applicability_domain(applicability_domain: ApplicabilityDomain,
                             train_features: pd.DataFrame,
                             test_features: pd.DataFrame = None,
                             folds: BaseCrossValidator = None,
                             groups: List[int] = None,
                             verbose: bool = False
                             ) -> pd.DataFrame:
    """Determine if samples lie in a model's applicability domain.

   :param applicability_domain: instalce of applicability domain to be used
   :param train_features: training set containing only covariates
   :param test_features: test set containing only covariates; ignored if None
   :param folds: cross-validator; if not None, the training set is cross-validated.
   ...
   :return: cross-validated performance and model trained on the entire dataset
    """
    steps = 1 if folds is None else folds.n_splits + 1
    if verbose:
        pbar = tqdm(desc='Determining applicability domain (through CV)', total=steps)
    performance = pd.Series()
    if folds is not None:
        # Perform cross-validation
        for i, (train, test) in enumerate(folds.split(train_features, groups=groups)):
            applicability_domain.fit(train_features.values[train])
            performance[f'Fold {i + 1}'] = sum(applicability_domain.contains(train_features.values[test])) / len(test)
            if verbose:
                pbar.update()
        # Add average and sd of  performance
        performance['Mean'] = np.mean(performance.iloc[:folds.n_splits])
        performance['SD'] = np.std(performance.iloc[:folds.n_splits])
    # Fit model on the entire dataset
    if verbose:
        pbar.set_description('Determining applicability to test set', refresh=True)
    if test_features is not None:
        # Evaluate the test set
        applicability_domain.fit(train_features.values)
        performance['Test'] = sum(applicability_domain.contains(test_features.values)) / test_features.shape[0]
    if verbose:
        pbar.update()
    return performance


def model_external_molecular_applicability_domains(ref_set_path: str, njobs: int = -1):
    """Determine the applicability domains of molecular data models of the entire set of bioactivities and of mutants only.
    Output results are stored under DATADIR/modelling

    :param ref_set_path: path to a feather file containing Mold2 descriptors of the reference molecules
    :param njobs: number of parallel processes; -1 uses all cores, -2 uses all but one, ...
    """
    # Set number of cores
    if njobs < 0:
    ...
    ref_dataset = pd.read_feather(ref_set_path)
    # Model accessions by decreasing number of datapoints
    sorted_accessions = list(zip(*sorted(Counter(data.accession).items(), key=itemgetter(1), reverse=True)))[0]
    pbar = tqdm(sorted_accessions, smoothing=0.0, ncols=120)
    for accession in pbar:
        # Determine names of all output files
        out_file_qsar_random_tsv = os.path.join(path, f'qsar_randomsplit_{accession}_external_applicability_domains.tsv')
        out_file_qsar_nomutant_random_tsv = os.path.join(path, f'qsar_nomutant_randomsplit_{accession}_external_applicability_domains.tsv')
        out_qsar_mutants = [out_file_qsar_random_tsv]
        out_qsar_no_mutants = [out_file_qsar_nomutant_random_tsv]
        all_files = list(chain(out_qsar_mutants, out_qsar_no_mutants))
        # Skip if all output files already exists
        if all(os.path.exists(f) for f in all_files):
            pbar.update()
            continue
        ...
        if not all(os.path.exists(f) for f in out_qsar_mutants):
            # 1) Isolate IDs and dependent variable
            ids = accession_data[['connectivity', 'target_id']]
            # 2) Keep unique mols and prots
            mols = accession_data[['connectivity', 'SMILES']].drop_duplicates()
            # 3) Obtain descriptors for unique mols
            mdesc_type = Mold2(False)
            ...
            if qsar_data.shape[0] >= 5:  # Number of folds
                ad = IsolationForestApplicabilityDomain(random_state=1234)
                test_ad = get_applicability_domain(ad, qsar_data, ref_dataset)
                # Save performances
                test_ad.to_csv(out_file_qsar_random_tsv, sep='\t')
        # Run QSAR with mutants removed
        if not all(os.path.exists(f) for f in out_qsar_no_mutants):
            # 1) Isolate IDs and dependent variable
            ids = accession_data[['connectivity', 'target_id']]
            ...
            if qsar_data.shape[0] >= 5:  # Number of folds
                ...
                test_ad = get_applicability_domain(ad, qsar_data, ref_dataset)
                # Save performances
                test_ad.to_csv(out_file_qsar_nomutant_random_tsv, sep='\t')


def model_common_subsets_external_applicability_domains(ref_set_path: str, njobs: int = -1):
    """Determine the applicability domains of molecular data models of the entire set of bioactivities and of mutants only.
    Output results are stored under DATADIR/modelling
    ...
    for common_set, accession in pbar:
        ...
        if accession_data.empty:
            ...
            continue
        # Determine names of all output files
        out_file_qsar_random_tsv = os.path.join(path, f'qsar_common_subset_randomsplit_{accession}_external_applicability_domains.tsv')
        out_file_qsar_nomutant_random_tsv = os.path.join(path, f'qsar_common_subset_nomutant_randomsplit_{accession}_external_applicability_domains.tsv')
        out_qsar_mutants = [out_file_qsar_random_tsv]
        out_qsar_no_mutants = [out_file_qsar_nomutant_random_tsv]
        all_files = list(chain(out_qsar_mutants, out_qsar_no_mutants))
        # Skip if all files already exists
